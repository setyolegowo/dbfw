...
typedef std::set<std::string> strmap;
...
public:
    ...
    void blockParseResponse(std::string & response);
    ...
    bool FindErrorInParse(const std::string& value){return parseerror.find(value) != parseerror.end();}
    void SetParseError(const std::string& value){parseerror.insert(value);}
    void ClearParseError(const std::string& value){parseerror.erase(value);}
