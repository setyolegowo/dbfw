...
bool removeSpaces(std::string & query)
{
    ...
    for (i = 0; i < query.size(); i++)
    {
        if (query[i] == ' ' && (query[i+1] == ')' || query[i+1] == ','))
            query.erase(i,1);
        if (i+1 <= query.size() && query[i+1] == ' ' && (query[i] == '(' || query[i] == ','))
            query.erase(i+1,1);
    }
...
static bool removeCppComment(std::string & query)
{
    ...
    for (i = 0; i< query.size()-1; i++)
    {
        ...
        if (query[i] == '/' && query[i+1] == '*') {
            for (j=i+2; j < query.size()-1 && query[j] != '*' && query[j+1] != '/'; j++)
            {
                // do nothing
            }
...
static bool removeHashComment(std::string & query)
{
    ...
    for (i = 0; i < query.size(); i++)
    {
        ...
        if (query[i] == '#')
        {
            for (j=i+1; j < query.size() && (query[j] != '\r' && query[j] != '\n'); j++)
            {
                // do nothing
            }
            for (; j < query.size() &&
...
static bool removeDashComment(std::string & query)
{
    ...
    for (i = 0; i< query.size()-1; i++)
    {
        // check for the start of the comment block
        if (query[i] == '-' && query[i+1] == '-') {
            for (j=i+2; j < query.size() && (query[j] != '\r' && query[j] != '\n'); j++)
            {
                // do nothing
            }
            for (; j < query.size() && (query[j] == '\r' || query[j] == '\n'); j++)
            {
                    // do nothing
            }
            //query[i] = ' ';
...
static bool fixNegativeNumbers(std::string & query)
{
    ...
    while ( (next = query.find_first_of(" ,(", prev)) != std::string::npos )
    {
