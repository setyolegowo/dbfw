...
bool SQLPatterns::compile_pattern(std::string & str, pcre ** _re, pcre_extra ** _pe ) {
    const char * error;
    int erroffset;
...
bool SQLPatterns::Match(MatchType type, const std::string & str)
{
    ...
    switch (type)
    {
        case SQL_ALTER:
            if (alter_str.length() == 0)
                return false;
            rc = pcre_exec(alter_re, alter_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_BLOCK:
            if (block_str.length() == 0)
                return false;
            rc = pcre_exec(block_re, block_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_CREATE:
            if (create_str.length() == 0)
                return false;
            rc = pcre_exec(create_re, create_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_DROP:
            if (drop_str.length() == 0)
                return false;
            rc = pcre_exec(drop_re, drop_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_INFO:
            if (info_str.length() == 0)
                return false;
            rc = pcre_exec(info_re, info_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_S_TABLES:
            if (s_tables_str.length() == 0)
                return false;
            rc = pcre_exec(s_tables_re, s_tables_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_EMPTY_PWD:
        ...
        case SQL_VAR_CMP:
            if (var_cmp_str.length() == 0)
                return false;
            rc = pcre_exec(var_cmp_re, var_cmp_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_TRUE_CONSTANTS:
            if (true_constants_str.length() == 0)
                return false;
            rc = pcre_exec(true_constants_re, true_constants_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        case SQL_BRUTEFORCE_FUNCTIONS:
            if (bruteforce_functions_str.length() == 0)
                return false;
            rc = pcre_exec(bruteforce_functions_re, bruteforce_functions_pe, str.c_str(), len, 0, 0, NULL, 0);
            break;
        default:
            return false;
    }
